...
const App = () => {
  ...
  function handleClickVote() {
    ...
    indexOfMax(counters)
    console.log('max',indexOfMax(counters))
  }
  ...
  function indexOfMax(arr) {
    return arr.reduce(
        (maxIndex, current, currentIndex, array) => {
            return current > 
            array[maxIndex] ? currentIndex : 
            maxIndex;
    }, 0);
}
  //{counters[selected]} <br></br>
  //{selected}
  //{indexOfMax(counters)}<br></br>
  //{counters}
  return (
    <div>
      ...
      {anecdotes[indexOfMax(counters)]} <br></br>
      has {counters[indexOfMax(counters)]} votes <br></br>
    </div>
  )
}
