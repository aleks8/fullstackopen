...
import {getAll, create, update, remove, createComment} from './services/blogs';
...
*/
  ...
  const addComment = (commentObject) => {
    ...
    //const idComment = id.toString()
    console.log('commment-in-create-comment', commentObject);
    //console.log('idComment', idComment)
    //const commentObject = {id:idComment, content: comment.content}
    //part of 7.11 answer 
    newBlogCommentMutation.mutate(commentObject);
...
*/
/*
<li><Link to={`/blogs/${blog.id}`}>{blog.title} {blog.author}{' '}</Link></li>
*/
  const blogStyle = {
    paddingTop: 10,
    paddingLeft: 2,
  ...
  const blogList = () => (
    ...
    <div>
      {blogs.map(blog => (
        ...
        <Link to={`/blogs/${blog.id}`}>{blog.title} {blog.author}{' '}</Link><br></br>
        </div>
        </div>
      ))}
    </div>
  ...
  return (
    ...
    <Router>
      ...
      <div style={{  padding: '10px' }}>
        ...
        <Link style={padding} to="/users">users</Link>
        {userValue ? 
          <form onSubmit={handleLogout}>{userValue.name} logged-in <Button primary='' type="submit">logout</Button>
          </form>
      ...
      <Routes>
        ...
        <Route path="/users/:id" element={<User usersForView={usersForView} />} />
...
const Home = ({addBlogForm, blogList}) => 
  ...
  const User = ({usersForView}) => {
    if (!usersForView) {    
      return null  }
      
    const id = useParams().id
    //console.log('idatuser', id)
    const user = usersForView.find(u => u.id === id)
    //console.log('useratuserhere', user)
    if(!user) {
      return null 
    }
    //console.log('useratuser', user)
      //<td>{user.blogs.length}</td>
    return (
      <div>
      <h2>{user.name}</h2>
  ...
  const BlogView = ({blogs, changeLikes, addComment}) => {
    if (!blogs) {    
      return null  }
    ...
    return (
      ...
      <Link>{blog.url}</Link>
      <br></br>
      {blog.likes} likes <Button primary=''onClick={()=>changeLikes(blog)}>like</Button>
      <br></br>
      added by {blog.user.name}
      <h3>comments</h3>
      <AddCommentForm id={blog.id} addComment={addComment}/>
